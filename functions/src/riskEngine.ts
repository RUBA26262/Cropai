...
export function calculateCropRisk(weather: RiskWeatherInput, context: CropRiskContext = {}) {
  ...
  return {
    ...
    scores: { disease: diseaseScore, pest: pestScore, weather: weatherScore, waterStress, overall },
    factors,
  };
}

export function buildScoutingMission(result: ReturnType<typeof calculateCropRisk>, context: CropRiskContext = {}) {
  const dominant = result.scores.pest >= result.scores.disease ? "pest" : "disease";
  ...
  return {
    urgency,
    headline: dominant === "pest" ? "Inspect for early pest pressure" : "Inspect for early disease symptoms",
    inspect: dominant === "pest"
      ? ["Field edges and plants near weeds", "Lower leaf surfaces and new shoots", "Damaged stems, buds and fruiting parts"]
      : ["Lower and inner leaves", "Dense or poorly ventilated patches", "Plants near standing water or irrigation lines"],
    capture: ["One close-up of the affected area", "One whole-plant view", "One wider field-context view"],
    questions: dominant === "pest"
      ? ["Are holes, mines, webbing or sticky residue visible?", "Are insects, eggs or droppings visible?", "Is damage increasing across nearby plants?"]
      : ["Are spots, lesions, mould or yellowing visible?", "Are symptoms spreading from lower leaves?", "Is the affected patch growing after rain or irrigation?"],
